...
2. **TTS engine:** ~~Decision needed before implementing AudiobookFlowDialog.~~ **Resolved:** Three engines behind the `TTSService` abstract interface, selected automatically:
   - **`NativeTTSService`** (built-in, no dependencies) — active fallback for all preset voices while Sherpa-ONNX model packaging is prepared. Generates a sinusoidal WAV file per voice profile without an external runtime; keeps the full audiobook flow exercisable end-to-end in the MVP. Produces audible but non-speech output — sufficient for flow testing, not for production listening. Swapped out transparently once Sherpa-ONNX is integrated (Task 13 delivered the `TTSService` interface; the swap requires only a new concrete subclass and CMake linkage).
   - **[Sherpa-ONNX](https://github.com/k2-fsa/sherpa-onnx)** (Apache 2.0) — target engine for all preset voices. Runs Piper-format `.onnx` + `.json` voice models natively; 40+ languages; actively maintained (v1.12.39, April 2026). C++ API with CMake support; ONNX Runtime is its core dependency.
   - **[PocketTTS.cpp](https://github.com/VolgaGerm/PocketTTS.cpp)** (MIT) — used only when (a) the book's language is supported (EN, FR, DE, IT, PT, ES) **and** (b) the user selects a custom voice. Zero-shot voice cloning from a short audio sample; 9.2× realtime on CPU; five shared ONNX model files ([models](https://huggingface.co/KevinAHM/pocket-tts-onnx)).

   The engine switch is transparent to the user — `AudiobookService` selects the backend; `AudiobookFlowDialog` shows the "Custom voice" option only when the active language is PocketTTS-supported. Progress reporting for all engines is chunk-based (timer-driven for `NativeTTSService`, local inference for Sherpa-ONNX/PocketTTS), so the `QProgressBar` model is identical across backends.
